...
// ==================== 捆绑 SSH 检测 ====================
function findBundledSSH() {
	const candidates = [];

	// Electron 打包: process.resourcesPath/lib/bin/ssh.exe
	if (process.resourcesPath) {
		candidates.push(path.join(process.resourcesPath, 'lib', 'bin', 'ssh.exe'));
	}

	// pkg 打包: 跟 exe 同级目录
	try {
		candidates.push(path.join(path.dirname(process.execPath), 'lib', 'bin', 'ssh.exe'));
	} catch (e) { /* ignore */ }

	// 开发模式: __dirname = lib/
	candidates.push(path.join(__dirname, 'bin', 'ssh.exe'));
...
function detectSSH() {
	...
	// 系统 SSH 未找到 → 回退到捆绑的 ssh.exe
	const plinkPath = findBundledSSH();
	if (plinkPath) {
		return { available: true, path: plinkPath, version: 'bundled', type: 'bundled' };
	}

	return { available: false, error: '未检测到 SSH 客户端 (系统未安装 OpenSSH，也未找到捆绑的 ssh.exe)' };
}
...
function startPinggyTunnel(localPort) {
	return new Promise((resolve) => {
		...
		// 捆绑的也是真 OpenSSH，参数统一
		const args = ['-p', '443', '-R', `0:localhost:${localPort}`, '-o', 'StrictHostKeyChecking=no', '-o', 'ServerAliveInterval=30', 'tcp@a.pinggy.io'];
...
function checkTunnelReady() {
	...
	const ssh = detectSSH();
	if (ssh.available) {
		const label = ssh.type === 'bundled' ? `捆绑SSH v${ssh.version}` : `OpenSSH v${ssh.version}`;
