...
function startPinggyTunnel(localPort) {
	return new Promise((resolve) => {
		...
		const args = isPlink
			? ['-P', '443', '-R', `0:localhost:${localPort}`, 'tcp@a.pinggy.io']
		...
		try {
		const spawnOpts = { windowsHide: true, env: { ...process.env } };
		if (isPlink) {
			// plink 需要手动接受 host key，用 pipe stdin 发送 "y"
			spawnOpts.stdio = ['pipe', 'pipe', 'pipe'];
			tunnelProcess = spawn(ssh.path, args, spawnOpts);
			tunnelProcess.stdin.write('y\n');
		} else {
