...
function detectSSH() {
	...
	for (const sshPath of candidates.filter(p => !seen.has(p.toLowerCase()) && seen.add(p.toLowerCase()))) {
		try {
			...
			return { available: true, path: sshPath, version: vMatch ? vMatch[1] : '未知' };
		} catch (e) {
			const errOut = (e.stderr || e.stdout || '').toString();
			const vMatch = errOut.match(/OpenSSH[_\s]([\d.]+)/i);
			if (vMatch) return { available: true, path: sshPath, version: vMatch[1] };
		}
	}
	return { available: false, error: '未检测到 SSH 客户端' };
}
...
function startPinggyTunnel(localPort) {
	return new Promise((resolve) => {
		const ssh = detectSSH();
		if (!ssh.available) {
			resolve({ success: false, backend: 'pinggy.io', message: `SSH 不可用: ${ssh.error || '未安装'}` });
			return;
		}

		console.log(`[Tunnel] 启动 pinggy.io TCP 隧道 (端口 ${localPort})...`);
		...
		try {
			...
			resolve({ success: false, backend: 'pinggy.io', message: `启动失败: ${err.message}` });
			return;
		}
...
function checkTunnelReady() {
	...
	const ssh = detectSSH();
	if (ssh.available) {
		return { ready: true, backend: 'pinggy.io', detail: `SSH v${ssh.version} 已就绪 (60分钟自动续期)` };
	}
